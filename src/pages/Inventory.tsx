    ...
    <>
      <header className="bg-[#131313] text-on-surface font-headline font-bold uppercase sticky top-0 z-40 bg-opacity-90 backdrop-blur-md">
        <div className="flex justify-between items-center w-full px-6 py-4">
          <h1 className="text-xl tracking-tight">Inventory Supervision</h1>
      ...
      <main className="px-6 mt-4 space-y-6">
        <div className="flex justify-between items-center bg-surface-container-low p-4 rounded-2xl mb-4">
          <div className="flex flex-col">
            <span className="text-sm font-bold text-on-surface-variant uppercase tracking-widest pt-1">All Assets</span>
            <span className="text-xs font-bold text-primary bg-primary/10 px-3 py-1 rounded-full uppercase pt-1.5 inline-block w-max mt-2">{equipment.length} Active</span>
          </div>
          ...
          {equipment.map(eq => (
            <div key={eq.id} className="bg-surface-container-low p-4 rounded-2xl border-l-4 overflow-hidden relative" style={{ borderColor: eq.operationalStatus === 'FUNCTIONAL' ? 'var(--primary)' : eq.operationalStatus === 'MAINTENANCE' ? 'var(--error)' : 'var(--outline)' }}>
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] bg-surface-container-highest px-2 py-0.5 rounded-sm uppercase tracking-wider text-on-surface-variant font-bold">{eq.type}</span>
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                <div>
                  <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest pt-1">Operation Cost</p>
                  <p className="font-headline text-sm font-bold">${eq.operatingCostPerHour}/hr</p>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest pt-1">Condition</p>
