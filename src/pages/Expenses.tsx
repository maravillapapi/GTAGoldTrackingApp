      ...
      <header className="bg-[#131313] text-on-surface font-headline font-bold uppercase sticky top-0 z-40 bg-opacity-90 backdrop-blur-md border-b border-outline/10">
        <div className="flex justify-between items-center w-full px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl tracking-tight">Expenses Log</h1>
          </div>
      ...
      <main className="px-6 mt-6 space-y-8">
        {role !== 'OBSERVER' && (
          <section className="bg-surface-container-low p-6 rounded-2xl shadow-lg border border-outline-variant/20">
            <h2 className="font-headline text-lg font-bold uppercase tracking-widest text-on-surface mb-4">Record Expense</h2>
            
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest pt-1">Amount ($)</label>
                <input 
                  type="number" 
                  step="0.01"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest pt-1">Category</label>
                <select 
                  className="bg-surface-container-highest border border-outline-variant/20 rounded-lg p-3 text-sm font-headline font-bold focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary w-full transition-colors"
                  value={category} 
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest pt-1">Description (Optional)</label>
                <textarea 
              ...
              <div className="flex flex-col gap-2 mt-2">
                <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Receipt / Attachment (Optional)</label>
                <div className="flex items-center gap-4">
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-headline text-sm font-bold uppercase tracking-widest text-on-surface-variant">Expense History</h2>
              ...
              <div key={exp.id} className="bg-surface-container-low p-4 rounded-2xl flex flex-col gap-3 border-l-2 border-error/50 hover:bg-surface-container-highest transition-colors">
                <div className="flex justify-between items-center">
                  <div className="flex gap-4 items-center">
                    ...
                    <div className="flex flex-col">
                      <h3 className="font-bold text-sm tracking-widest uppercase">{exp.category}</h3>
                ...
                {(exp.description || exp.receiptImageUrl) && (
                  <div className="mt-2 pt-3 border-t border-outline/10 text-xs text-on-surface-variant flex justify-between items-start gap-4">
                    <p className="flex-1">{exp.description || 'No description provided.'}</p>
                    {exp.receiptImageUrl && (
                      <div className="w-16 h-16 rounded overflow-hidden flex-shrink-0 border border-outline/20">
                        <img src={exp.receiptImageUrl} alt="Attachment" className="w-full h-full object-cover" />
                      </div>
                    )}
                  </div>
                )}
