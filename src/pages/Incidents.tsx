      ...
      <main className="px-6 mt-4 space-y-8">
        {role !== 'OBSERVER' && (
          <section className="bg-surface-container-low p-6 rounded-2xl shadow-lg border-l-4 border-error">
            <h2 className="font-headline text-sm font-bold uppercase tracking-widest text-on-surface mb-4">Log New Incident</h2>
            
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest pt-1">Severity</label>
                <select 
        ...
        <section className="space-y-4">
          <h2 className="font-headline text-sm font-bold uppercase tracking-widest text-on-surface-variant border-b border-outline/10 pb-2">Active Log</h2>
            ...
            {[...incidents].reverse().map(inc => (
              <Link to={`/incidents/${inc.id}`} key={inc.id} className={`bg-surface-container-low p-4 rounded-2xl flex flex-col gap-2 border-l-4 decoration-transparent hover:bg-surface-container-highest transition-colors cursor-pointer ${inc.severity === 'CRITICAL' || inc.severity === 'HIGH' ? 'border-error' : inc.severity === 'MEDIUM' ? 'border-primary' : 'border-outline/30'}`}>
                <div className="flex justify-between items-start">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase pt-1 ${inc.status === 'OPEN' ? 'bg-error/10 text-error' : 'bg-surface-container-highest text-on-surface-variant'}`}>{inc.status}</span>
                </div>
                ...
                <div className="flex justify-between items-center mt-2 border-t border-outline/10 pt-3">
                  ...
                  <span className={`text-[10px] font-bold uppercase tracking-widest ${inc.severity === 'CRITICAL' ? 'text-error' : 'text-primary'}`}>{inc.severity} Priority</span>
                </div>
              </Link>
            ))}
            {incidents.length === 0 && (
              <p className="text-sm font-body text-on-surface-variant text-center my-8">No incidents in the system.</p>
            )}
          </div>
        </section>
      </main>
